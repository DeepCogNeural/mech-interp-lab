...
> - [Amendment 4](#amendment-4--2026-08-08--q4-matched-pool-data-role-separation)
...
## Amendment 4 — 2026-08-08 — Q4 matched-pool data-role separation

**Written after Stage 1 was run and read, after Amendment 3 and its independent Advisor review, and
before either Stage 2 or Stage 3 has run.** The review found that disjoint experiment seeds protect the
twelve target rows from experiment 04's selection data but do not, by themselves, specify independent
data roles for constructing and evaluating each Stage-3 seed's matched-latent reference pool. This
amendment closes that ambiguity before any Q4 result exists.

For every Stage-3 seed, reuse experiment 04's item-disjoint split rule after Gate A: 40 retained minimal
pairs form the **rank-training split**, and the remaining retained pairs form the **evaluation split**,
capped at 150. A minimal pair and both of its directed edits stay in exactly one split. The split is
generated by experiment 04's existing procedure under that Stage-3 experiment seed, and the manifest
must record the seed, every retained pair id and its split assignment.

The union-of-active-coordinates construction, cheap causal pre-filter, causal ranking, and freeze of the
128-candidate matched pool may read only the rank-training split. The twelve target rows are excluded as
already required. Once frozen, the pool may not be reordered, replaced, trimmed, or have its thresholds
changed using any evaluation-split result.

The target twelve-row `R_span` and every matched-subset `R_span`, including their common `E(δ)`
denominator, are computed only on the held-out evaluation split for that seed. Drawing and full-rank
checks remain independent of evaluation effects: use `default_rng(experiment_seed × 1000 + 401)`, retain
100 full-rank matched subsets, and record the frozen pool ids, all drawn subset ids, and all rejected
rank-deficient subset ids. If either split cannot meet its stated count after Gate A, the seed is blocked
rather than allowing pairs to cross data roles.

This is a data-role clarification only. It does not change the Stage-3 seeds `20260806…20260813`, the
twelve target rows, target exclusion, float64 SVD projector, rank tolerance, within-subset sampling
