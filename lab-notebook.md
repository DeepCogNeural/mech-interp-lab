...
**Next:** A calibration pilot restricted to residual-level interventions — it measures the two specificity constants and, separately, the per-patch cost and the per-seed fixed overhead. Separately, because experiment 04's pilot under-projected the main run by 25× by scaling per-patch cost alone, when 71% of the runtime turned out to be overhead that does not scale with patches. Those constants go into `DESIGN.md`, and the commit containing them is the freeze.
