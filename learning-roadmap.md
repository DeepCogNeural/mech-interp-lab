...
## Current plan (updated 2026-08-13)

Five experiments are now complete and a sixth has a result-blind protocol and implementation reviewed
interactively by an AI advisor (not external expert validation); it remains unrun with no final-SHA receipt
and has no result; the audits are in `lab-notebook.md`. The finding that reordered this
roadmap on 2026-07-26 was that **every result up to that point was linear decodability of a representation,
and none of it was causal.** Week 3 below is the part that had been skipped and it was the part that
mattered most.

`experiments/04_causal_feature_interchange/` closed part of that gap: a causal interchange intervention on GPT-2-small, readout on the model's own logits, run under a frozen pre-registration. Its rule returned **inconclusive** — a faithfulness gate committed before any commit containing this experiment's output was missed by 0.006 — so experiment 04 remained a causal *basis comparison* rather than a mechanism result. Experiment 05 has now crossed that boundary: its fresh selection, Stage-2 intervention, and independent Stage-3/Q4 span comparison all ran, with Q1–Q4 positive in eight of eight seeds.

That distinction sets what comes next.

1. **The number-agreement circuit — [Stage 2, Stage 3/Q4, and bridge complete](experiments/05_number_agreement_circuit/writeup.md).** Fresh true-source and source-A sweeps selected eight candidate heads from one unchanged model snapshot. Across eight adjudication seeds, the minimum tested prefix `[L7H4, L8H5]` recovered `0.521–0.546` of the registered direct effect, both heads were individually distinguishable, both specificity ratios cleared their frozen limits, and the frozen-pattern value-path effect was positive with every bootstrap interval above zero. The independent Q4 comparison then found that the frozen 12-row layer-8 span retained `R_span = 0.8935` of the directed logit effect, exceeded the frozen matched-span edge in every seed, and left `R_comp = 0.0848` in the complement. A single exploratory bridge on eight fresh seeds recovered `R_target = 0.6786` of the L7H4→`resid_pre8` effect and beat all 100 matched rank-12 spans per seed; when the complete L8H5 `hook_z` output at the final query position was overwritten, it remained `0.6740`. That clamp is post-attention aggregation and is not value-only. The licensed conclusion is compact, number-selective causal structure plus a reproducible causal subspace under the tested interventions—not natural/monosemantic latent activations, individual-latent causality, necessity, sufficiency, dominant L8H5 mediation, a complete circuit, or cross-model/task generalisation. The proposed all-position-clamp factorial was withdrawn as non-identifying: the upstream perturbation changes no non-final `resid_pre8` position, and causal masking leaves no earlier L8H5 query output to disambiguate.
2. **[Experiment 06 — fixed-object cross-template bridge](experiments/06_cross_template_bridge/DESIGN.md), reviewed/implemented and unrun.** Hold exact GPT-2/SAE revisions and file hashes, L7H4, the fixed 12-row span, and the ordinal 100-span matched controls constant; evaluate them on the calibration-exposed relative-clause family under a globally fixed cyclic source-A map. Because the earlier bridge used seed-drawn source-A nouns, this is a second prompt/control construction, not a one-factor template-only contrast. Require all eight seeds, first adjudicate the fixed true-versus-source-A handle, and only then compare the span with the second-largest raw-effect edge over the frozen Q4 matched sets. This reuses Q4's sets and order statistic, not its normalized `R` estimand. Gate-A population failure is `NON_ESTIMABLE`; implementation or provenance guard failure is `STOPPED`. No output or transfer claim exists yet.
3. **Week 3 proper — activation patching and IOI**, on whatever tooling that experiment forces me to build, so the machinery arrives attached to a question rather than as an exercise.
4. **Week 5–6 — train an SAE.** Everything so far uses a published one. Training a small one is the only way to separate an SAE's objective from its particular training run.
5. Then the deferred items: re-registering experiment 04's Gate C on a decoder each arm actually ships with, a larger PCA fit, shattering/CCGP on experiment 02's toy geometries, and the Gram-interference mechanism check.
