...
1. **The number-agreement circuit — [calibration and the shipped Stage-1 scope complete; selection-only source-A supplement and Stages 2–3 not started](experiments/05_number_agreement_circuit/DESIGN.md).** The experiment asks which attention heads move the subject-number signal under registered interventions and whether experiment 04's twelve recurring layer-8 decoder rows span more of the transported delta than matched 12-dimensional spans. The shipped Stage-1 artifact contains the true-source 144-head `z@final` sweep, `E_all`, `E_ref`, and top-24 `v@subject` measurements; it adjudicates none of Q1–Q4. Before Stage 2, seed `20260801` still requires the selection-only full source-A sweep that completes candidate set `C`; Stage 3 has not run. The eventual claims are intervention-specific rather than assertions that the heads or latents belong to the model's native causal path. Bootstrap and source-A sampling uncertainty can narrow with more retained pairs, while Q2 uses fixed calibration-derived specificity ratios and Q4 uses a finite matched-span reference distribution.
