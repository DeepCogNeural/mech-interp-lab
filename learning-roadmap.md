...
1. **The number-agreement circuit — [calibration and the shipped Stage-1 scope complete; result-free runners implemented; fresh selection and Stages 2–3 not run](experiments/05_number_agreement_circuit/DESIGN.md).** The experiment asks which attention heads move the subject-number signal under registered interventions and whether experiment 04's twelve recurring layer-8 decoder rows span more of the transported delta than matched 12-dimensional spans. The shipped Stage-1 artifact contains the true-source 144-head `z@final` sweep, `E_all`, `E_ref`, and top-24 `v@subject` measurements; it adjudicates none of Q1–Q4. Because that artifact does not retrospectively prove the exact model snapshot needed for candidate selection, Amendment 7 requires fresh true-source and source-A 144-head sweeps in one invocation, with matching before/after model-state fingerprints; the shipped ranking is only a descriptive cross-check. That 291-logical-FE selection has not run, so candidate `C` does not exist and Stages 2–3 remain unstarted. The eventual claims are intervention-specific rather than assertions that the heads or latents belong to the model's native causal path. Bootstrap and source-A sampling uncertainty can narrow with more retained pairs, while Q2 uses fixed calibration-derived specificity ratios and Q4 uses a held-out finite matched-span reference distribution.
