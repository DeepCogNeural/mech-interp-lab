...
## Current plan (updated 2026-08-09)

Five experiments are now complete; the audits are in `lab-notebook.md`. The finding that reordered this
...
1. **The number-agreement circuit — [Stage 2, Stage 3/Q4, and bridge complete](experiments/05_number_agreement_circuit/writeup.md).** Fresh true-source and source-A sweeps selected eight candidate heads from one unchanged model snapshot. Across eight adjudication seeds, the minimum tested prefix `[L7H4, L8H5]` recovered `0.521–0.546` of the registered direct effect, both heads were individually distinguishable, both specificity ratios cleared their frozen limits, and the frozen-pattern value-path effect was positive with every bootstrap interval above zero. The independent Q4 comparison then found that the frozen 12-row layer-8 span retained `R_span = 0.8935` of the directed logit effect, exceeded the frozen matched-span edge in every seed, and left `R_comp = 0.0848` in the complement. A single exploratory bridge on eight fresh seeds recovered `R_target = 0.6786` of the L7H4→`resid_pre8` effect and beat all 100 matched rank-12 spans per seed; under the L8H5 final-value clamp it remained `0.6740`. The licensed conclusion is compact, number-selective causal structure plus a reproducible causal subspace under the tested interventions—not natural/monosemantic latent activations, individual-latent causality, necessity, sufficiency, dominant L8H5 mediation, a complete circuit, or cross-model/task generalisation. The next frozen research question is a small factorial distinguishing a too-narrow clamp from an L8H5-parallel route; it is planned, not run.
